...
- `InteractionBehavior` — clase base (`Resource`, `class_name`) con `verbo`, `puede_interactuar(actor, objeto) -> bool` e `interactuar(actor, objeto) -> void`. Cada comportamiento concreto (`PoseBehavior`, `ContenedorBehavior`, `EquiparBehavior`...) es una subclase pequeña y reutilizable de esto.
- `ItemDefinition` gana un campo `interacciones: Array[InteractionBehavior]`. Agregar un verbo a un ítem nuevo casi nunca pide código: se arrastra un `.tres` de comportamiento ya existente (ej. el mismo `sentarse.tres` sirve para todas las sillas) al array del ítem, en línea con el principio de §8 de que contenido nuevo no debería requerir tocar código.
